...
console.log("🚀 Adding Meat Pie Calculator...\n");

// 1. Create MeatPieCalculator.tsx
const meatPieCode = `"use client"

import { useState, useEffect } from "react";
import { useSession } from "next-auth/react";

export default function MeatPieCalculator() {
  const [pieDetails, setPieDetails] = useState({ size: "Standard", quantity: 1 })
  const [ingredients, setIngredients] = useState({ flour: 0, butter: 0, meat: 0, potatoes: 0, carrots: 0, eggs: 0, seasoning: 0 })
  ...
  const ingredientTotal = Number(ingredients.flour) + Number(ingredients.butter) + Number(ingredients.meat) + Number(ingredients.potatoes) + Number(ingredients.carrots) + Number(ingredients.eggs) + Number(ingredients.seasoning)
  ...
  const totalCost = (ingredientTotal + businessTotal + packagingTotal) * pieDetails.quantity
  ...
  const downloadSummary = async () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(18); doc.text("Meat Pie Pricing Summary", 20, 20);
    doc.setFontSize(12);
    doc.text(\`Size: \${pieDetails.size}\`, 20, 40);
    doc.text(\`Quantity: \${pieDetails.quantity}\`, 20, 50);
    ...
    doc.save(\`meatpie-summary-\${Date.now()}.pdf\`);
  };
  ...
  const saveCalculation = async () => {
    try {
      const response = await fetch("/api/save-calculation", {
        ...
        body: JSON.stringify({ pieDetails, ingredients, businessCosts, packaging, totalCost, expectedProfit, sellingPrice }),
      })
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8 mt-10">
      <div className="bg-white p-6 rounded-2xl shadow">
        <h2 className="text-2xl font-bold mb-4">🥟 Meat Pie Costs</h2>
        <div className="space-y-4">
          <select className="w-full border p-3 rounded-lg" value={pieDetails.size} onChange={(e) => setPieDetails({ ...pieDetails, size: e.target.value })}>
            <option>Standard</option><option>Jumbo</option><option>Party Pack (Mini)</option>
          </select>
          <label className="block text-sm font-semibold mb-2">Quantity (e.g., 50 pieces)</label>
          <input type="number" placeholder="Quantity" className="w-full border p-3 rounded-lg" value={pieDetails.quantity} min={1} onChange={(e) => setPieDetails({ ...pieDetails, quantity: Number(e.target.value) })} />
          ...
          <input type="number" placeholder="Minced Meat/Beef Cost" value={ingredients.meat === 0 ? "" : ingredients.meat} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, meat: e.target.value === "" ? 0 : Number(e.target.value) })} />
          <input type="number" placeholder="Potatoes Cost" value={ingredients.potatoes === 0 ? "" : ingredients.potatoes} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, potatoes: e.target.value === "" ? 0 : Number(e.target.value) })} />
          <input type="number" placeholder="Carrots Cost" value={ingredients.carrots === 0 ? "" : ingredients.carrots} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, carrots: e.target.value === "" ? 0 : Number(e.target.value) })} />
          ...
          <input type="number" placeholder="Seasoning/Curry Cost" value={ingredients.seasoning === 0 ? "" : ingredients.seasoning} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, seasoning: e.target.value === "" ? 0 : Number(e.target.value) })} />
          ...
          <button type="button" className="w-full bg-red-500 text-white p-3 rounded-lg hover:opacity-90" onClick={() => {
            setIngredients({ flour: 0, butter: 0, meat: 0, potatoes: 0, carrots: 0, eggs: 0, seasoning: 0 });
            setBusinessCosts({ transport: 0, labor: 0, electricity: 0 });
            setPackaging({ bags: 0, cartons: 0 });
            setProfitPercentage(20);
            setPieDetails({ size: "Standard", quantity: 1 });
      ...
      <div id="summary" className="bg-[#5c3d2e] text-white p-6 rounded-2xl shadow">
        ...
        <div className="space-y-4 text-lg">
          <p>Size: {pieDetails.size}</p>
          <p>Quantity: {pieDetails.quantity}</p>
...
fs.writeFileSync('components/MeatPieCalculator.tsx', meatPieCode);
console.log("✅ Created MeatPieCalculator.tsx");

// 2. Update ProductSelector.tsx to include Meat Pie
let selectorCode = fs.readFileSync('components/ProductSelector.tsx', 'utf8');
...
selectorCode = selectorCode.replace(
  /<button\s+onClick=\{\(\) => setActiveTab\("doughnut"\)\}[\s\S]*?🍩 Doughnut Calculator\s*<\/button>/,
  newButton
);
...
console.log("✅ Updated ProductSelector.tsx with Meat Pie tab");

console.log("\n🎉 Meat Pie added! Run 'npx tsc --noEmit' to verify.");
