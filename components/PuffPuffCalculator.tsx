...
export default function PuffPuffCalculator() {
  const [puffDetails, setPuffDetails] = useState({ type: "Standard", quantity: 1 })
  const [ingredients, setIngredients] = useState({ flour: 0, sugar: 0, yeast: 0, nutmeg: 0, waterMilk: 0, oil: 0 })
  ...
  const ingredientTotal = Number(ingredients.flour) + Number(ingredients.sugar) + Number(ingredients.yeast) + Number(ingredients.nutmeg) + Number(ingredients.waterMilk) + Number(ingredients.oil)
  ...
  const totalCost = (ingredientTotal + businessTotal + packagingTotal) * puffDetails.quantity
  ...
  const downloadSummary = async () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(18); doc.text("Puff Puff Pricing Summary", 20, 20);
    doc.setFontSize(12);
    doc.text(`Type: ${puffDetails.type}`, 20, 40);
    doc.text(`Quantity: ${puffDetails.quantity}`, 20, 50);
    ...
    doc.save(`puffpuff-summary-${Date.now()}.pdf`);
  };
  ...
  const saveCalculation = async () => {
    try {
      const response = await fetch("/api/save-calculation", {
        ...
        body: JSON.stringify({ puffDetails, ingredients, businessCosts, packaging, totalCost, expectedProfit, sellingPrice }),
      })
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8 mt-10">
      <div className="bg-white p-6 rounded-2xl shadow">
        <h2 className="text-2xl font-bold mb-4">🍘 Puff Puff Costs</h2>
        <div className="space-y-4">
          <select className="w-full border p-3 rounded-lg" value={puffDetails.type} onChange={(e) => setPuffDetails({ ...puffDetails, type: e.target.value })}>
            <option>Standard</option><option>Mini / Party Size</option><option>Jumbo</option>
          </select>
          <label className="block text-sm font-semibold mb-2">Quantity (e.g., 100 pieces or batches)</label>
          <input type="number" placeholder="Quantity" className="w-full border p-3 rounded-lg" value={puffDetails.quantity} min={1} onChange={(e) => setPuffDetails({ ...puffDetails, quantity: Number(e.target.value) })} />
          ...
          <input type="number" placeholder="Nutmeg Cost" value={ingredients.nutmeg === 0 ? "" : ingredients.nutmeg} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, nutmeg: e.target.value === "" ? 0 : Number(e.target.value) })} />
          <input type="number" placeholder="Water/Milk Cost" value={ingredients.waterMilk === 0 ? "" : ingredients.waterMilk} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, waterMilk: e.target.value === "" ? 0 : Number(e.target.value) })} />
          ...
          <button type="button" className="w-full bg-red-500 text-white p-3 rounded-lg hover:opacity-90" onClick={() => {
            setIngredients({ flour: 0, sugar: 0, yeast: 0, nutmeg: 0, waterMilk: 0, oil: 0 });
            setBusinessCosts({ transport: 0, labor: 0, electricity: 0 });
            setPackaging({ bags: 0, cartons: 0 });
            setProfitPercentage(20);
            setPuffDetails({ type: "Standard", quantity: 1 });
      ...
      <div id="summary" className="bg-[#5c3d2e] text-white p-6 rounded-2xl shadow">
        ...
        <div className="space-y-4 text-lg">
          <p>Type: {puffDetails.type}</p>
          <p>Quantity: {puffDetails.quantity}</p>
