...
export default function BananaBreadCalculator() {
  const [bananaBreadDetails, setBananaBreadDetails] = useState({ type: "Classic", quantity: 1 })
  const [ingredients, setIngredients] = useState({ bananas: 0, flour: 0, sugar: 0, butter: 0, eggs: 0, bakingPowder: 0, vanilla: 0, cinnamon: 0, milk: 0 })
  const [businessCosts, setBusinessCosts] = useState({ transport: 0, labor: 0, electricity: 0 })
  const [packaging, setPackaging] = useState({ loafPans: 0, bags: 0 })
  ...
  const ingredientTotal = Number(ingredients.bananas) + Number(ingredients.flour) + Number(ingredients.sugar) + Number(ingredients.butter) + Number(ingredients.eggs) + Number(ingredients.bakingPowder) + Number(ingredients.vanilla) + Number(ingredients.cinnamon) + Number(ingredients.milk)
  const businessTotal = Number(businessCosts.transport) + Number(businessCosts.labor) + Number(businessCosts.electricity)
  const packagingTotal = Number(packaging.loafPans) + Number(packaging.bags)

  const totalCost = (ingredientTotal + businessTotal + packagingTotal) * bananaBreadDetails.quantity
  ...
  const downloadSummary = async () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(18); doc.text("Banana Bread Pricing Summary", 20, 20);
    doc.setFontSize(12);
    doc.text(`Type: ${bananaBreadDetails.type}`, 20, 40);
    doc.text(`Quantity: ${bananaBreadDetails.quantity}`, 20, 50);
    ...
    doc.save(`bananabread-summary-${Date.now()}.pdf`);
  };
  ...
  const saveCalculation = async () => {
    try {
      const response = await fetch("/api/save-calculation", {
        ...
        body: JSON.stringify({ bananaBreadDetails, ingredients, businessCosts, packaging, totalCost, expectedProfit, sellingPrice }),
      })
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8 mt-10">
      <div className="bg-white p-6 rounded-2xl shadow">
        <h2 className="text-2xl font-bold mb-4">🍌 Banana Bread Costs</h2>
        <div className="space-y-4">
          <select className="w-full border p-3 rounded-lg" value={bananaBreadDetails.type} onChange={(e) => setBananaBreadDetails({ ...bananaBreadDetails, type: e.target.value })}>
            <option>Classic</option><option>Chocolate Chip</option><option>Nut & Banana</option><option>Swirl (Cinnamon)</option>
          </select>
          <label className="block text-sm font-semibold mb-2">Quantity (e.g., number of loaves)</label>
          <input type="number" placeholder="Quantity" className="w-full border p-3 rounded-lg" value={bananaBreadDetails.quantity} min={1} onChange={(e) => setBananaBreadDetails({ ...bananaBreadDetails, quantity: Number(e.target.value) })} />

          <h3 className="font-bold text-gray-700 mt-4">Ingredients Cost (₦)</h3>
          <input type="number" placeholder="Ripe Bananas/Plantains Cost" value={ingredients.bananas === 0 ? "" : ingredients.bananas} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, bananas: e.target.value === "" ? 0 : Number(e.target.value) })} />
          ...
          <input type="number" placeholder="Baking Powder/Soda Cost" value={ingredients.bakingPowder === 0 ? "" : ingredients.bakingPowder} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, bakingPowder: e.target.value === "" ? 0 : Number(e.target.value) })} />
          <input type="number" placeholder="Vanilla Essence Cost" value={ingredients.vanilla === 0 ? "" : ingredients.vanilla} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, vanilla: e.target.value === "" ? 0 : Number(e.target.value) })} />
          <input type="number" placeholder="Cinnamon/Nutmeg Cost" value={ingredients.cinnamon === 0 ? "" : ingredients.cinnamon} className="w-full border p-3 rounded-lg" onChange={(e) => setIngredients({ ...ingredients, cinnamon: e.target.value === "" ? 0 : Number(e.target.value) })} />
          ...
          <input type="number" placeholder="Electricity/Gas (Oven)" value={businessCosts.electricity || ""} className="w-full border p-3 rounded-lg" onChange={(e) => setBusinessCosts({ ...businessCosts, electricity: e.target.value === "" ? 0 : Number(e.target.value) })} />

          <h3 className="font-bold text-gray-700 mt-4">Packaging (₦)</h3>
          <input type="number" placeholder="Loaf Pans/Aluminum Trays Cost" value={packaging.loafPans || ""} className="w-full border p-3 rounded-lg" onChange={(e) => setPackaging({ ...packaging, loafPans: e.target.value === "" ? 0 : Number(e.target.value) })} />
          ...
          <button type="button" className="w-full bg-red-500 text-white p-3 rounded-lg hover:opacity-90" onClick={() => {
            setIngredients({ bananas: 0, flour: 0, sugar: 0, butter: 0, eggs: 0, bakingPowder: 0, vanilla: 0, cinnamon: 0, milk: 0 });
            setBusinessCosts({ transport: 0, labor: 0, electricity: 0 });
            setPackaging({ loafPans: 0, bags: 0 });
            setProfitPercentage(20);
            setBananaBreadDetails({ type: "Classic", quantity: 1 });
      ...
      <div id="summary" className="bg-[#5c3d2e] text-white p-6 rounded-2xl shadow">
        ...
        <div className="space-y-4 text-lg">
          <p>Type: {bananaBreadDetails.type}</p>
          <p>Quantity: {bananaBreadDetails.quantity}</p>
